...
required_packages <- c("rmarkdown", "gprofiler2", "data.table", "ggplot2", "igraph", "reticulate", "plotly", "tidyr", "dplyr", "GGally", "ComplexHeatmap", "circlize", "readxl", "ggpattern")
...
data_info <- original_data_genes$meta_info
...
#ggsave(paste0(in_vis_out_dir, "pie_plots_studies.png"), width = 12, height = 4.5)
```
...
Upon harmonization, the next step was to select commonly affected proteins (CAPs) that are consistently present across multiple studies. 
...
## Calculate Weights and Intersections
...
library(openxlsx)
openxlsx::write.xlsx(networks_tissues_data, file = paste(out_dir, "network_tissues_data.xlsx"))
...
```{r}
literature_results <- read_excel("data/literature_versand.xlsx", sheet = "Tabelle1") # check if current version with SERPINA1 = beneficial
