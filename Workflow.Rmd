...
out_tmp <- paste0(out_dir, "/temp_pie_charts")
dir.create(out_tmp)
sapply(names(tissue_annotation[!tissue_annotation %in% c("new", "problem")]), function(gene){
  ...
  ggsave(paste0(out_tmp, "/", gene, ".png"), width = 1, height = 1)
})

pie_charts_files <- list.files(out_tmp, full.names = TRUE)


# rearrange vector for problem and new 
names(pie_charts_files) <- sapply(list.files(out_tmp), function(x) strsplit(x, ".png")[[1]])
#names_new_problem_genes <- names(tissue_annotation[tissue_annotation %in% c("new", "problem")])
...
tissue_anno_vec <- c(pie_charts_files, new_problem_genes)
...
png(paste0(out_dir, "scoring_heatmap.png"), width=9,height=12,units="in",res=1200)
draw(scoring_heatmap, annotation_legend_list = list(heatmap_legend, digest_legend, drug_legend, literature_legend, type_legend, tissue_legend), annotation_legend_side = "right", ht_gap = unit(0.5, "cm"), auto_adjust = TRUE)
dev.off()
```
