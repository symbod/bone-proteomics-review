...
for (type in unique(intersections$Type)) {
  
  # Subset the dataframe
  subset_df <- intersections[intersections$Type == type, ]
  ...
  # Save the plot
  ggsave(paste0(in_vis_out_dir, "barchart_plot_", type, ".png"), plot = result[[1]], device = "png", width = 10, height = 8, units = "in")
  
  # Save the dataframe
  write.csv(result[[2]], file = paste0(in_vis_out_dir, "barchart_data_", type, ".csv"), row.names = FALSE)
}
```
...
for (type in unique(filtered_intersections$Type)) {
  ...
  p_without_drugs <- ggnet2(de_results_types[["ami"]][[type]]$ds_graph, size = "degree", color = "node_attributes", shape = "node_type", 
              color.palette = color_palette, label = TRUE, label.size = 2) + guides(size = FALSE) + 
  scale_shape_manual(values = c("circle", "triangle"), labels = c("seed", "exception")) 
  ggsave(paste0(ne_vis_out_dir, type, "-ami_graph.png"), p_without_drugs, width = 30, height = 15, units = "cm", dpi = 150)
}
...
```{r}
#top_genes_network <- extract_top_genes_union(network_scoring)$top_genes_union
#top_genes_enrich <- extract_top_genes_union(enrich_scoring)$top_genes_union

#all_top_genes <- unique(c(top_genes_network, top_genes_enrich))

all_top_genes <- drug_scores[drug_scores$drugs != 0,]$ID

# Select data for these genes from both scorings
network_data_selected <- network_scoring[network_scoring$IDs %in% all_top_genes, ]
enrich_data_selected <- enrich_scoring[enrich_scoring$IDs %in% all_top_genes, ]

# Merge the selected data
combined_data <- merge(network_data_selected, enrich_data_selected, by = "IDs", all = TRUE)

cols <- c("IDs", grep("_Rank$", names(combined_data), value = TRUE))
combined_data <- combined_data[, ..cols]

plot_mat <- as.matrix(combined_data[,-1], rownames=combined_data$IDs)

drug_annotation <- drug_scores[match(rownames(plot_mat), drug_scores$IDs), "drugs"]
type_annotation <- igraph::as_data_frame(core_drugs_ds$ds_graph, what= "vertices")["node_attributes"]
type_annotation <- type_annotation[match(rownames(plot_mat), rownames(type_annotation)), "node_attributes"]


drug_colors <- colorRampPalette(c("white", "red"))(max(drug_annotation) + 1)
names(drug_colors) <- 0:max(drug_annotation)
ha <- rowAnnotation(Drugs = anno_simple(drug_annotation, pch = paste(drug_annotation), col = drug_colors),
                    Type = anno_simple(type_annotation, col = color_palette, pch = type_annotation, pt_size = unit(0.5, "cm"), width = max_text_width(month.name)*1.2))
...
gene_drug_matrix <- do.call(cbind, lapply(drugs_list, function(drug_genes) {
  +(rownames(plot_mat) %in% drug_genes)
}))
rownames(gene_drug_matrix) <- rownames(plot_mat)
...
gene_distance_matrix <- as.dist(1 - proxy::simil(gene_drug_matrix, method = "correlation"))

# 4. Cluster the Genes
gene_clustering <- hclust(gene_distance_matrix)

col_fun <- colorRamp2(c(1, 10, max(plot_mat)), c("darkgreen", "orange", "grey"))

pdf( paste0(ne_vis_out_dir, "top_genes_combined_heatmap.pdf"))#, width = 2000, height = 3000), res = 300) # Adjust dimensions as needed
...
heatmap <- Heatmap(
  ...
  cell_fun = function(j, i, x, y, width, height, fill) {
    grid.text(plot_mat[i, j], x, y, gp = gpar(fontsize = 8)) # Adjust fontsize as needed
  },
  heatmap_legend_param = list(
    title = "Measure Rank",
    at = c(1, 10, max(plot_mat)),
    labels = c("1 (Best)", "10", paste0(max(plot_mat)," (Worse)")),
